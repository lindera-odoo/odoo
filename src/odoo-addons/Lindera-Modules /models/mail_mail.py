    ...
    def send(self, auto_commit=False, raise_exception=False):
        ...
        for id in ids:
            ...
            blacklistMails = ['service@lindera.de', 'support@lindera.odoo.com',
                              'vendor-bills@lindera.odoo.com', 'invoices@lindera.de']
            ...
            if token_backend.check_token() and allowtosend:
                try:
                    ...
                    if account.is_authenticated:
                        ...
                        for link in re.findall(r'/web/(?:content|image)/([0-9]+)', body):
                            attachments = attachments - \
                                IrAttachment.browse(int(link))

                        # load attachment binary data with a separate read(), as prefetching all
                        # `datas` (binary field) could bloat the browse cache, triggerring
                        # soft/hard mem limits with temporary data.
                        attachments = [(BytesIO(base64.b64decode(a['datas'])), a['datas_fname']) for a in attachments.sudo(
                        ).read(['datas_fname', 'datas']) if a['datas'] is not False]
                        ...
                        if mail.email_to:
                            ...
                            values['email_to'] = tools.email_split(
                                mail.email_to)
                            ...
                            if mail.parent_id:
                                ...
                                if mail.parent_id.o365ID:
                                    ...
                                    if prev_mail:
                                        mail.parent_id = prev_mail[0]
                                    try:
                                        oldMessage = mailbox.get_message(
                                            mail.parent_id.o365ID)
                        ...
                        _logger.info(
                            'Mail with ID %r successfully sent', mail.id)
                        mail._postprocess_sent_message(
                            success_pids=process_pids)
                except Exception as e:
                    ravenSingle.Client.captureMessage(e)
                    ...
                    mail._postprocess_sent_message(
                        failure_reason=failure_reason, failure_type='UNKNOWN')
                    if raise_exception:
                        if isinstance(e, (AssertionError, UnicodeEncodeError)):
                            ...
                            raise MailDeliveryException(
                                ("Mail Delivery Failed"), value)
                        raise
            else:
                super(linderaMail, mail).send(
